...
async def scrapping_loker(request: ScrapeRequest, _: dict = Depends(require_admin)):
    data = scrape_loker_jobs(request.query, request.max_page, max_jobs=request.max_jobs)
    return {"query": request.query, "max_page": request.max_page, "max_jobs": request.max_jobs, "data": data}

@router.post("/scrapping/glints")
async def scrapping_glints(request: ScrapeRequest, _: dict = Depends(require_admin)):
    data = scrape_glints_jobs(keyword=request.query, end_page=request.max_page, max_jobs=request.max_jobs)
    return {"query": request.query, "max_page": request.max_page, "max_jobs": request.max_jobs, "data": data}

@router.post("/scrapping/jobstreet")
async def scrapping_jobstreet(request: ScrapeRequest, _: dict = Depends(require_admin)):
    data = scrape_jobstreet_jobs(request.query, request.max_page, max_jobs=request.max_jobs)
    return {"query": request.query, "max_page": request.max_page, "max_jobs": request.max_jobs, "data": data}